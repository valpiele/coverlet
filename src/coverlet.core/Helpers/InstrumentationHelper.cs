        ...
        private (bool allDocumentsMatch, string notFoundDocument) MatchDocumentsWithSources(MetadataReader metadataReader)
        {
            foreach (DocumentHandle docHandle in metadataReader.Documents)
            {
                ...
                if (!_fileSystem.Exists(docName) 
                    && !docName.EndsWith(".g.cs") 
                    && !docName.EndsWith("AssemblyInfo.cs")
                    && !docName.EndsWith("AssemblyAttributes.cs")
                    && !Path.GetFileNameWithoutExtension(docName).StartsWith(".NETCoreApp,")
